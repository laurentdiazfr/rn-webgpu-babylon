# Babylon.js with react-native-webgpu

A minimal demo running Babylon.js on React Native via WebGPU using [react-native-webgpu](https://github.com/wcandillon/react-native-webgpu).
...
- **`context.present()`** — Calling `context.present()` in the render loop right after `scene.render()` causes issues. It’s done instead in an observer; `onBeforeRenderObservable` is the only place I found that works without WebGPU warnings.
